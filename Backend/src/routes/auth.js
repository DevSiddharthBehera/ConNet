...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = new User({ username, password: hashed, displayName: displayName || username });
    await user.save();
...
async function findOrCreateOAuthUser({ provider, id, name, email }) {
  ...
  let user = await User.findOne({ $or: [{ username }, { 'oauth.provider': provider, 'oauth.providerId': id }] });
  if (!user) {
    user = new User({ username, password: await bcrypt.hash(Math.random().toString(36), 8), displayName: name || username, oauth: { provider, providerId: id, email } });
    await user.save();
  }
  return user;
}
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    const userInfoResp = await axios.get(`https://www.googleapis.com/oauth2/v2/userinfo?access_token=${tokenResp.data.access_token}`, { headers: { Authorization: `Bearer ${tokenResp.data.access_token}` } });
    const info = userInfoResp.data; // { id, email, name, picture }

    const user = await findOrCreateOAuthUser({ provider: 'google', id: info.id, name: info.name, email: info.email });
    const token = jwt.sign({ id: user._id.toString(), username: user.username, displayName: user.displayName }, JWT_SECRET, { expiresIn: '7d' });

    // If opened as popup, postMessage back to opener and close
    if (req.query.popup) {
      const frontend = process.env.FRONTEND_URL || 'http://localhost:5174';
      return res.send(`<!doctype html><html><body><script>window.opener.postMessage(${JSON.stringify({ token, user: { id: user._id.toString(), username: user.username, displayName: user.displayName } })}, '${frontend}');window.close();</script></body></html>`);
    }
