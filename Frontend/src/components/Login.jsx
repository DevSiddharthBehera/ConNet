...
export default function Login({ onAuth }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${isRegister ? "Registered" : "Logged in"} as ${user.username}`,
        status: "success",
        duration: 2000,
      });
      onAuth(token, user);
    ...
    >
      <VStack spacing={4} align="stretch">
        ...
        <HStack spacing={3} justify="center">
          <Button onClick={() => openOAuth("google")} colorScheme="red">
            Sign in with Google
          </Button>
        </HStack>

        <Text fontSize="sm" color="gray.500" textAlign="center">
          This is a demo. Use unique usernames for testing.
        </Text>
      </VStack>
    </Box>
  );
