...
function initSocket(io) {
  ...
  io.on('connection', async (socket) => {
    ...
    } catch (err) { console.error('Room auto-join error', err) }
    ...
    io.emit('user-online', { id: user.id, username: user.username, displayName: user.displayName });
    ...
    socket.on('message', async (payload) => {
      ...
      try {
        ...
        let isGroup = false;
        try { if (await Room.findById(to).select('_id')) isGroup = true; } catch (e) { /* ignore */ }
        const msgDoc = new Message({ from: user.id, to, content, meta, file });
        await msgDoc.save();
        const out = { id: msgDoc._id.toString(), from: user, to, content, meta, file, createdAt: msgDoc.createdAt, isGroup };
            ...
            try {
              ...
              if (roomMembers && roomMembers.members) {
                roomMembers.members.forEach(memberId => {
                  ...
                  if (memberStr !== user.id) io.to(memberStr).emit('message', out);
                });
              }
            } catch (err) { console.error('Group member emit error', err) }
        }
      } catch (err) {
    ...
    socket.on('file', async (payload) => {
      ...
      try {
        ...
        await msgDoc.save();
        const fileOut = { from: user, fileName, mimeType, buffer, id: msgDoc._id.toString(), createdAt: msgDoc.createdAt };
    ...
    socket.on('call-user', ({ to, offer }) => {
      ...
      if (targetSet) {
        targetSet.forEach(targetSocketId => io.to(targetSocketId).emit('incoming-call', { from: user, offer }))
      }
    });
    ...
    socket.on('answer-call', ({ to, answer }) => {
      const targetSet = onlineUsers.get(to);
      if (targetSet) {
        targetSet.forEach(targetSocketId => io.to(targetSocketId).emit('call-answered', { from: user, answer }))
      }
    });
    ...
    socket.on('ice-candidate', ({ to, candidate }) => {
      const targetSet = onlineUsers.get(to);
      if (targetSet) {
        targetSet.forEach(targetSocketId => io.to(targetSocketId).emit('ice-candidate', { from: user, candidate }))
      }
    });
    ...
    socket.on('end-call', ({ to }) => {
      ...
      const targetSet = onlineUsers.get(to);
      if (targetSet) {
        targetSet.forEach(targetSocketId => io.to(targetSocketId).emit('call-ended', { from: user }))
      }
      // Notify all of caller's other sockets (multi-tab) so they clean up too
      const callerSet = onlineUsers.get(user.id);
      if (callerSet) {
        callerSet.forEach(sid => { if (sid !== socket.id) io.to(sid).emit('call-ended', { from: user }) })
      }
    });
    ...
    socket.on('mark-read', async ({ conversationId }) => {
      try {
        ...
        await Message.updateMany(
          { to: conversationId, from: { $ne: user.id }, 'meta.readBy': { $ne: user.id } },
