...
export default function VideoModal({ isOpen, onClose, localStream, remoteStream, recipient, onEndCall }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Call with {recipient?.displayName || recipient?.id}</ModalHeader>
        <ModalBody>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ width: '100%' }}>
              <p style={{ fontSize: '14px', fontWeight: 500, margin: '0 0 8px 0' }}>Remote</p>
              <video 
                ref={remoteVideoRef}
                playsInline
                autoPlay
                style={{ width: '100%', height: '300px', background: '#000', borderRadius: '8px', objectFit: 'cover', display: 'block' }} 
              />
            </div>
            <div style={{ width: '100%' }}>
              <p style={{ fontSize: '14px', fontWeight: 500, margin: '0 0 8px 0' }}>Local</p>
              <video 
                ref={localVideoRef}
                playsInline
                autoPlay
        ...
        <ModalFooter>
          <Button colorScheme="red" onClick={onEndCall}>End Call</Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  )
