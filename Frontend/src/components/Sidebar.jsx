...
import { Box, VStack, Avatar, Text, Input, Divider, HStack, Button, Spinner, IconButton, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, ModalCloseButton, FormControl, FormLabel, Checkbox, Stack, useDisclosure } from '@chakra-ui/react'
...
export default function Sidebar({ token, user, selected, onSelect, isMobile }) {
  ...
  useEffect(() => {
    ...
    s.on('message', (msg) => {
      ...
      const isActive = selectedRef.current && selectedRef.current.id === conversationId
  ...
  async function fetchUnreadCounts() {
    try {
      const resp = await API.get('/messages/unread/counts', { headers: { Authorization: `Bearer ${token}` } })
  ...
  async function fetchLists() {
    ...
    try {
      ...
      const fetchedUsers = (uRes.data || []).map(u => ({ ...u, id: u.id || u._id }))
      const fetchedRooms = (rRes.data || []).map(r => ({ ...r, id: r.id || r._id }))
      ...
      if ((!selected || !selected.id) && onSelect && !isMobile) {
        ...
        convs.sort((a, b) => {
          ...
          if (allZero) {
            const myId = String(user?.id || user?._id)
            const nonSelf = convs.find(c => c.type === 'user' && String(c.id) !== myId)
  ...
  async function fetchLatestMessages(userList, roomList) {
    try {
      ...
      for (const u of userList) {
        try {
          const resp = await API.get(`/messages/${u.id}?limit=1`, { headers: { Authorization: `Bearer ${token}` } })
          if (resp.data && resp.data.length > 0) {
            const msg = resp.data[0]
            timestamps[u.id] = new Date(msg.createdAt || msg.created_at).getTime()
            const fromId = String(msg.from?.id || msg.from?._id || msg.from)
            const content = msg.content || msg.text || ''
            contents[u.id] = fromId === myId ? `You: ${content}` : content
          }
        } catch (err) { /* ignore individual fetch errors */ }
      }
      for (const r of roomList) {
        try {
          const resp = await API.get(`/messages/${r.id}?limit=1`, { headers: { Authorization: `Bearer ${token}` } })
          if (resp.data && resp.data.length > 0) {
            const msg = resp.data[0]
            timestamps[r.id] = new Date(msg.createdAt || msg.created_at).getTime()
            const fromId = String(msg.from?.id || msg.from?._id || msg.from)
            const content = msg.content || msg.text || ''
  ...
  const conversations = [
    ...
    ...rooms.map(r => ({ ...r, type: 'room', displayName: r.name || 'Group' }))
  ]
      ...
      <VStack align="stretch" spacing={4}>
        ...
        <Box>
          <HStack justify="space-between" mb={2}>
            ...
            <IconButton size="sm" aria-label="Create group" icon={<AddIcon />} onClick={openCreate} />
          </HStack>
          {loading ? <Spinner /> : (
            <VStack align="stretch" spacing={2} maxH="480px" overflowY="auto">
              {filteredConversations.map(c => {
                const isOnline = c.type === 'user' && onlineIds.includes(String(c.id))
                const unreadCount = unreadCounts[c.id] || 0
                const isSelf = c.type === 'user' && String(c.id) === String(user.id)
                return (
                  <Box key={c.id} p={2} borderWidth={selected?.id === c.id ? 2 : 1} borderRadius="md" borderColor={selected?.id === c.id ? 'blue.400' : 'gray.100'} _hover={{ bg: 'gray.50', cursor: 'pointer' }} onClick={() => {
                    onSelect({ id: c.id, displayName: c.displayName, isGroup: c.type === 'room' })
                    // Mark as read
                    ...
                    <HStack spacing={2} align="start">
                      <Box position="relative">
                        ...
                        {isOnline && !isSelf && (
                          <Box position="absolute" bottom={0} right={0} w={2.5} h={2.5} bg="green.400" borderRadius="full" border="2px solid white" />
                        )}
                      </Box>
                      <Box flex={1} minW={0}>
                        <HStack justify="space-between">
                          <Text fontWeight="medium" fontSize="sm">{c.displayName}</Text>
                          {!isSelf && unreadCount > 0 && (
                            <Box bg="blue.500" color="white" borderRadius="full" minW={5} h={5} display="flex" alignItems="center" justifyContent="center" fontSize="xs" px={1.5}>
                              {unreadCount > 99 ? '99+' : unreadCount}
                            </Box>
                          )}
                        </HStack>
                        <Text fontSize="xs" color="gray.500" noOfLines={1} isTruncated>
                          {isSelf ? 'Self (notepad)' : (lastMessageContent[c.id] || (c.type === 'room' ? `${c.members?.length || 0} members` : 'No messages yet'))}
                        </Text>
                      </Box>
                    </HStack>
                  </Box>
        ...
        <Divider />
        <HStack>
          <Button size="sm" onClick={fetchLists}>Refresh</Button>
        </HStack>
      </VStack>
    </Box>
      ...
      <ModalContent>
        ...
        <ModalBody>
          <FormControl mb={3}>
            ...
            <Input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="e.g. Friends" />
          </FormControl>
          ...
          <FormControl>
            ...
            <Stack spacing={2} maxH="200px" overflowY="auto">
              {users.map(u => (
                <Checkbox key={u.id} isChecked={groupMembers.includes(u.id)} onChange={e => {
                  const checked = e.target.checked
                  setGroupMembers(prev => checked ? [...prev, u.id] : prev.filter(id => id !== u.id))
                }}>{u.displayName || u.username}</Checkbox>
              ))}
            </Stack>
          </FormControl>
        </ModalBody>
        ...
        <ModalFooter>
          <Button variant="ghost" mr={3} onClick={closeCreate}>Cancel</Button>
          <Button colorScheme="blue" onClick={async () => {
            try {
              setLoading(true)
              const resp = await API.post('/rooms', { name: groupName, members: groupMembers }, { headers: { Authorization: `Bearer ${token}` } })
